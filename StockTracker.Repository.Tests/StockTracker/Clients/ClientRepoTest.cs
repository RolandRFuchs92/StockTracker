...
{
	[TestClass]
	public class ClientsRepoTest : TestUtils<ClientRepo>
	{
		...
		private const string _listAll = nameof(IClientRepo.GetAll);
		private const string _listActive = nameof(IClientRepo.GetAll);
		...
		[TestMethod]
		public void ListAll_Passnothin_ReturnAllClientsInDb()
		{
			//Arrange
			var repo = GetRepo();
			var resultCount = _db.Clients.Count();

			//Act
			repo.CreateResult(_listAll);

			//Assert
			Assert.AreEqual(resultCount, Result<List<IClient>>().Count);
			_log.NoLog();
		}

		#endregion

		#region List All Active
		[TestMethod]
		public void ListAllActive_PassNothing_GetListOfAllActiveClientsNoLog()
		{
			//Arrange
			var repo = GetRepo();

			//Act
			repo.CreateResult(_listActive);
